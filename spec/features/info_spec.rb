...
feature "Info page" do
  ...
  scenario "Seeing metrics for multi-part formats" do
    ...
    within("#per-page-metrics") do
      ...
      expect(page.all('td.page')[0].text).to eq('/apply-uk-visa/part-1')
      expect(page.all('td.page')[1].text).to eq('/apply-uk-visa/part-2')
      expect(page.all('td.page')[2].text).to eq('/apply-uk-visa/part-3')
      expect(page.all('td.unique_pageviews')[0].text).to eq('25k per day')
      expect(page.all('td.unique_pageviews')[1].text).to eq('24k per day')
      expect(page.all('td.unique_pageviews')[2].text).to eq('26k per day')
      expect(page.all('td.searches')[0].text).to eq('4k per day')
      expect(page.all('td.searches')[1].text).to eq('5k per day')
      expect(page.all('td.searches')[2].text).to eq('6k per day')
      expect(page.all('td.problem_reports')[0].text).to eq('1.4k per week')
      expect(page.all('td.problem_reports')[1].text).to eq('1.75k per week')
      expect(page.all('td.problem_reports')[2].text).to eq('700 per week')
    end
